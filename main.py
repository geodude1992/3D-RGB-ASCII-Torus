...
# Displays donut ASCII chars in RGB
def text_display(letter, x_start, y_start):
    text = font.render(str(letter), True, hsv2rgb(hue, 1, 1))
    display_surface.blit(text, (x_start, y_start))


# Displays donut ASCII chars in white
"""
def text_display(letter, x_start, y_start) :
...
while run:
    ...
    for j in range(0, 628, theta_spacing):  # from 0 to 2pi
        for i in range(0, 628, phi_spacing):  # from 0 to 2pi
            ...
            y = int(y_offset + 20 * D * (l * h * n + t * m))  # 3D y-coord after rotation
            o = int(x + columns * y)  # 3D z-coord after rotation
